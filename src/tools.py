...
def D_s_2(img_fake, img_lm, pan, satellite='QuickBird', scale=4, block_size=64, q=1):
    """Spatial distortion
...
def D_s(pan, ms, fused, q=1, r=4, ws=7):
    ...
    diff = np.abs(M1 - M2) ** q
    print('D_s:M1-M2:',np.abs(M1 - M2),'M1:',np.sum(M1),'M2:',np.sum(M2))
...
def D_lambda(ms, fused, p=1):
    ...
    diff = np.abs(M1 - M2) ** p
    print('D_lambda:M1-M2:',np.abs(M1 - M2),'M1:',np.sum(M1),'M2:',np.sum(M2))
